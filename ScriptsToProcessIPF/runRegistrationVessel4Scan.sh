...
if [ $# -ne 8 ]; then
  echo "Usage: $0 subject CT_input mask_input output_base scan moving_phase scan fixed_phase"
  exit 1
fi
...
reg_output_path="${output_base}/RegistrationWithVessel/${scanM}_${moving}_to_${scanF}_${fixed}/"
mkdir -p $reg_output_path


#######################################
# Input data to registration code is set.
moving_input="${reg_output_path}"
fixed_input="${CT_input}/${scanF}"

moving_image="${moving_input}/${subject}_${scanM}_${moving}_rigid_to_${scanF}_${fixed}.nii.gz"
fixed_image="${fixed_input}/${subject}_${scanF}_${fixed}.nii.gz"

moving_vm="NOINIT"
fixed_vm="NOINIT"

moving_mask_input="${reg_output_path}"
fixed_mask_input="${mask_input}/${scanF}/lung/alpha/"

moving_mask="${moving_mask_input}/${subject}_${scanM}_${moving}_rigid_to_${scanF}_${fixed}.mask.nii.gz"
...
modelfile="${param_model_path}/parameters.txt" # Path to model parameter file
